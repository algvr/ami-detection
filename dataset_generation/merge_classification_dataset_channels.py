...
# if you wish to use data augmentation, first augment the channel-wise dataset using "classification_dataset_augmentation.py",
# then merge the *augmented* dataset's channels


if __name__ == '__main__':
    ptb_v_dir = os.path.join('datasets', 'ptb_v', 'training')
    ptb_v_classification_dir = os.path.join('datasets', 'ptb_v_classification_aug', 'training')
    ptb_v_classification_merged_dir = os.path.join('datasets', 'ptb_v_classification_merged_aug', 'training')
    ...
    for filename in os.listdir(ptb_v_classification_dir):
        ...
        try:
            ...
            channel_idx = int(fn_root.replace('.png_aug', '').replace('.png', '').split('=')[-1])
        except:
            continue
        ...
        imgs_to_merge = [Image.open(os.path.join(ptb_v_classification_dir, '='.join(fn_root.split('=')[:-1])
                                                 + f'={channel_idx}{ext}' + ('_aug.png' if 'png_aug' in fn_root else ''))) for channel_idx in range(3)]
        ...
        json_path = os.path.join(ptb_v_dir, os.path.splitext(filename.split('__')[2])[0].replace('_aug', '') + '_data.json')
