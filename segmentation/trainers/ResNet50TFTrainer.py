...
class ResNet50TFTrainer(TFTrainer):
    """
    Trainer for the ResNet50TF model.
    """
    ...
    def __init__(self, dataloader, model, experiment_name=None, run_name=None, split=None, num_epochs=None,
        ...
        if optimizer_or_lr is None:
            optimizer_or_lr = ResNet50TFTrainer.get_default_optimizer_with_lr(lr=1e-4)
        elif isinstance(optimizer_or_lr, int) or isinstance(optimizer_or_lr, float):
            optimizer_or_lr = ResNet50TFTrainer.get_default_optimizer_with_lr(lr=optimizer_or_lr)
        ...
        preprocessing =\
            lambda x, is_gt: K.applications.resnet50.preprocess_input(tf.cast(x[:, :, :3], dtype=tf.float32)) if not is_gt \
            else (x[:, :, 0] // 85)
    ...
    def _get_hyperparams(self):
        return {**(super()._get_hyperparams()),
                **({param: getattr(self.model, param)
                   for param in ['dropout', 'kernel_init', 'pooling', 'num_final_fc_units', 'head_structure', 'dropout',
                                 'backbone_model', 'train_backbone', 'use_imagenet_weights']
